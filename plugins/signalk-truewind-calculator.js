...
module.exports = function(app) {
  ...
  function computeAndPublish(cfg) {
    try {
      ...
      // Build values array — ground-referenced (always)
      var vals = [
      ...
      // Water-referenced true wind (v1.0.4)
      var stw = getVal('navigation.speedThroughWater') || 0;
      var leeway = getVal('navigation.leewayAngle') || 0;
      var hdg_lw = ht + leeway;
      var vbwN = stw * Math.cos(hdg_lw);
      var vbwE = stw * Math.sin(hdg_lw);
      var vtwwN = v_aw_N + vbwN;
      var vtwwE = v_aw_E + vbwE;
      var tws_w = Math.sqrt(vtwwN * vtwwN + vtwwE * vtwwE);
      if (isFinite(tws_w) && tws_w * MS_TO_KTS <= cfg.maxWindKts) {
        var twd_w = ((Math.atan2(vtwwE, vtwwN) + Math.PI) % TWO_PI + TWO_PI) % TWO_PI;
        var twa_w = twd_w - ht;
        while (twa_w > Math.PI) twa_w -= TWO_PI;
        while (twa_w < -Math.PI) twa_w += TWO_PI;
        if (isFinite(twa_w)) {
          vals.push({ path: 'environment.wind.angleTrueWater', value: twa_w });
          vals.push({ path: 'environment.wind.speedTrue', value: tws_w });
          if (cfg.debug) svcLog('DEBUG',
            'WaterTW: TWA_w=' + (twa_w*RAD_TO_DEG).toFixed(1)
            + 'deg TWS_w=' + (tws_w*MS_TO_KTS).toFixed(1)
            + 'kts STW=' + (stw*MS_TO_KTS).toFixed(2)
            + 'kts leeway=' + (leeway*RAD_TO_DEG).toFixed(1) + 'deg');
        }
      }
