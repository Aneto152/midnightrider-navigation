...
class TestTestContentProvider(unittest.TestCase):
    ...
    def test_under_max_length(self):
        """Content must be under MAX_LENGTH."""
        provider = TestContentProvider()
        content = provider.get_content("test-race", "2026-08-26T18:00:00")
        
        self.assertLessEqual(len(content), TestContentProvider.MAX_LENGTH)
    ...
    def test_validate_rejects_too_long(self):
        """validate must reject content exceeding MAX_LENGTH."""
        provider = TestContentProvider()
        long_content = "é" * (TestContentProvider.MAX_LENGTH + 1)
