...
from regatta.fleet_stars_handler import (
    ...
    merge_starred, is_starred, set_store_path
)
...
class TestFleetStarsGlobalSync(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        """Create temporary isolated store for all tests"""
        cls.temp_dir = tempfile.TemporaryDirectory()
        cls.temp_store = Path(cls.temp_dir.name) / "fleet_stars_test.json"
        set_store_path(str(cls.temp_store))

    @classmethod
    def tearDownClass(cls):
        """Clean up temporary store"""
        set_store_path(None)  # Reset to production path
        cls.temp_dir.cleanup()

    def test_01_initial_store_empty(self):
        """Initial starred list is empty"""
        # Store starts empty for each test run
        starred = get_starred()
