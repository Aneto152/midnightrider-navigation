        ...
        while (buffer.length >= packetSize) {
          if (buffer[0] === 0x55 && buffer[1] === 0x61) {
            ...
            try {
              ...
              const accelX = (accelXRaw / 32768) * 16 * 9.81  // Convert g to m/s²
              const accelY = (accelYRaw / 32768) * 16 * 9.81
              const accelZ = (accelZRaw / 32768) * 16 * 9.81
              
              // GYROSCOPE (int16, /32768 × 2000 °/s)
              const gyroZRaw = packet.readInt16LE(12)   // Z-axis (Rate of Turn) at offset 12
              
              const gyroZ = (gyroZRaw / 32768) * (2000 * Math.PI / 180)  // Convert °/s to rad/s
              ...
              // Scale: 32768 = ±180°
              const roll = (rollRaw / 32768) * Math.PI
              const pitch = (pitchRaw / 32768) * Math.PI
              const yaw = (yawRaw / 32768) * Math.PI
